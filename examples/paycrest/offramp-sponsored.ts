...
async function main() {
  ...
  // The submission ran through `wallet.tx().paycrestOfframp(...).send(...)`
  // above (so we never get an `OfframpResult` with a built-in `wait()`).
  // Poll the gateway endpoint manually using the on-chain gateway_id
  // we extract from the receipt.
