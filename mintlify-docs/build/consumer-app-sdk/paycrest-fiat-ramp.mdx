...
<Note>
  **API path** is sponsored automatically by the Paycrest aggregator (it batches the approve + create_order server-side and submits via AVNU). You don't need to pass `feeMode`, and there are no Cartridge policies to register — the SDK only emits a single ERC20 `transfer` to the per-order receive address.
</Note>
